...
namespace TestSample.Microsoft
{
    ...
    internal class ODataRouteDebugVersionedMiddleware
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ODataRouteDebugVersionedMiddleware"/> class.
        /// </summary>
        /// <param name="routePattern">The route pattern.</param>
        /// <param name="next">The next middleware.</param>
        public ODataRouteDebugVersionedMiddleware(string routePattern, RequestDelegate next)
        {
        ...
        internal static IList<EndpointRouteInfo> GetRouteInfo(HttpContext context)
        {
            ...
            foreach (var endpoint in dataSource.Endpoints)
            {
                ...
                var metadata = endpoint.GetODataRoutingMetadata();

                if (metadata != null)
                {
                    var apiVersion = metadata.GetODataApiVersion();
                    if (apiVersion == null || !endpoint.IsODataApiVersionMatch(apiVersion))
                    {
                        continue;
                    }
                }
            ...
            routInfoList.Sort((one, other) => string.Compare(one.Pattern, other.Pattern, StringComparison.OrdinalIgnoreCase));
